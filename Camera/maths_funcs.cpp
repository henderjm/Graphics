...
mat4 translate_y(const mat4& m, const vec3& v) {
	mat4 m_t = identity_mat4();
	m_t.m[13] = v.v[1];
}
...
mat4 perspective (float fovy, float aspect, float x, float y) {
	float fov_rad = fovy * ONE_DEG_IN_RAD;
	float range = tan (fov_rad / 2.0f) * x;
	float sx = (2.0f * x) / (range * aspect + range * aspect);
	float sy = x / range;
	float sz = -(y + x) / (y - x);
	float pz = -(2.0f * y * x) / (y - x);
...
*/
//finish this!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
mat4 ortho(GLfloat l, GLfloat r, GLfloat b, GLfloat t, GLfloat f, GLfloat n){
	...
	tmp.m[12] = -1.0f*((r + l)/(r - l));
	tmp.m[13] = -1.0f*((t + b)/(t - b));
	tmp.m[14] = -1.0f*((f + n)/(f - n));
/*
	m.m[7] = -1*((r + l)/(r - l));
	m.m[3] = -1*((t + b)/(t - b));
	m.m[11] = -1*((f + n)/(f - n));
